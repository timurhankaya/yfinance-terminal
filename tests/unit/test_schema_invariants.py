"""Invariants that must hold across the whole schema: these scan the ENTIRE
`Base.metadata`, because a policy defined in one helper does not protect a table written
without it."""
...
def test_every_symbol_fk_uses_the_same_policy() -> None:
    """ON UPDATE CASCADE + ON DELETE RESTRICT: RESTRICT enforces the soft-delete policy at
    the DB level, so a single DELETE cannot wipe a symbol's history."""
    found = _symbol_fks()
...
def test_every_symbol_column_shares_the_symbols_collation() -> None:
    """An FK column's collation must exactly match its parent's. PostgreSQL raises no error
    on a mismatch; the drift silently splits JOIN semantics between parent and child."""
...
def test_no_timestamp_column_loses_sub_second_precision() -> None:
    """All timestamps are TIMESTAMP(6) WITH TIME ZONE. Second precision would collide on
    the (symbol, fetched_at) PK of ticker_info_history, and a naive column still looks
    correct because psycopg interprets it against the connection's TZ."""
    offenders = [
...
# PK prefixes allowed to precede `as_of_date` in as-of tables: `symbol`, `domain_key`, or
# `query_term` (a search term's result spans symbols), with regional domain tables inserting
# `region` in between. A "value as of day D" query still uses the prefix.
ASOF_PK_PREFIXES = (
    ("symbol",),
    ("domain_key",),
    ("domain_key", "region"),
...
def test_as_of_date_is_always_the_second_key_component() -> None:
    """If as_of_date is part of the key, it must be the second component, or a "value as of
    day D" query becomes a full scan. Gate tables are out of scope: there, as_of_date is
    data the gate carries, not part of the key."""
    checked = 0
    for table in Base.metadata.tables.values():
...
def test_child_tables_inherit_their_parent_timestamp() -> None:
    """Every timestamp-less table is either derived from a parent (`financial_facts` from
    `financial_periods`) or source-dated (`dividends`); a new one must state which."""
    source_dated = {
        "price_history",
        "dividends",
        "splits",
    ...
    exempt = {
        ...
        # `screens` is static identity like `symbols`: seeded from screens.py, not fetched.
        # The daily fetch timestamp lives on screen_runs.fetched_at.
        "screens",
...
# Reserved/dangerous words that look tempting as column names. Kept portable across MySQL
# and PostgreSQL: a name that works unquoted on both can never be broken by raw SQL. The
# ORM quotes its own SQL, so a reserved name only surfaces in hand-written queries and
# migrations. `status` is NOT on this list.
...
def test_no_column_uses_a_reserved_word() -> None:
    """Column names are never chosen from reserved words (`last_reported`, `rank_index`
    stand in for `reported`, `rank`)."""
    offenders = [
        name
        for table in Base.metadata.tables.values()
...
#
# `alembic check` cannot see this: autogenerate compares no CHECK constraints and cannot
# read an expression index back, while conftest's `create_all` emits everything the models
# declare. So a constraint can be enforced in tests and absent from a migrated database.
