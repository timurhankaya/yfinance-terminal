...
from yfin.pipeline.audit import ItemRecord, failed_records, record_items, skipped_records
...
def test_symbol_side_records_leave_region_null() -> None:
    ...
    records = record_items(dataset, "AAPL", _stats(), fetched=3, duration_ms=10)
    assert records
...
def test_market_side_records_leave_region_null() -> None:
    ...
    records = record_items(dataset, "US", stats, fetched=5, duration_ms=7)
    assert all(r.region is None for r in records)
...
def test_failed_records_without_region_are_unchanged() -> None:
    records = failed_records("AAPL", "info", "boom")
    assert all(r.region is None for r in records)
...
def test_domain_records_carry_the_region() -> None:
    ...
    records = record_items(dataset, "^YH311", stats, fetched=51, duration_ms=12, region="GB")
