...
def test_copy_path_writes_rows(writer: StreamWriter, db_session: Session) -> None:
    ...
    with writer._session_factory() as session:
        written = writer._write_ticks(session, [_row()]).written
        session.commit()
    assert written == 1
...
def test_a_batch_writes_in_one_statement(writer: StreamWriter, db_session: Session) -> None:
    ...
    with writer._session_factory() as session:
        written = writer._write_ticks(session, rows).written
        session.commit()
    assert written == 51
...
def test_a_batch_of_only_unknown_symbols_writes_nothing(
    ...
    with writer._session_factory() as session:
        written, unknown, _ = writer._write_ticks(session, [_row("NOPE")])
        assert written == 0
...
def test_verification_counts_what_is_in_the_table(
    ...
    with writer._session_factory() as session:
        assert writer._write_ticks(session, [_row()]).written == 1
...
def test_verification_still_counts_correctly_with_the_range(
    ...
    with writer._session_factory() as session:
        assert writer._write_ticks(session, rows).written == 3
