...
const GROUPS: ReadonlyArray<[title: string, codes: string[]]> = [
  ...
  ["Charts and the tape", ["GP", "QR"]],
  ["Several symbols at once", ["WLA", "COMP"]],
  ["Market-wide (no symbol; these live under /ui/m/)", ["HOME", "EQS", "HEAT", "CAL", "MKT", "DOM"]],
  ["Pages (these live under /ui/w/)", ["PG"]],
...
const EXAMPLES: ReadonlyArray<[command: string, what: string]> = [
  ...
  ["GP", "Candles for the symbol on the strip: daily, two years, with dividends and splits marked."],
  ["GP 1d 5", "The same chart over five years. A window in years, above intraday."],
  ["AAPL GP 5m", "Five-minute candles for the last few sessions, with the archive's gaps shaded."],
  ["GP 1wk", "Weekly candles. One function for every interval the archive keeps."],
  ...
  ["PG", "Every page saved in this browser, and which function key opens it."],
  ["PG SAVE trading", "Names the page you are on: it becomes /ui/w/trading and survives a reload."],
  ["PG trading", "Opens that page. F1-F4 and F7-F10 do the same, in PG's order."],
  ["SHARE", "Writes the page you are on as a link, layout and all. Nothing is sent anywhere."],
  ["GRP B", "Pins the focused panel to group B: typing a symbol then moves every panel wearing B."],
